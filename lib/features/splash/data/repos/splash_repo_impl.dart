import 'package:firebase_auth/firebase_auth.dart';
import 'package:swap_skill/core/services/firebase_auth_services.dart';
import 'package:swap_skill/core/services/local_storage_services.dart';
import 'package:swap_skill/features/splash/data/repos/splash_repo.dart';

class SplashRepoImpl implements SplashRepo {
  SplashRepoImpl({required this.firebaseAuthServices, required this.localStorageServices});
  final LocalStorageServices localStorageServices;

  final FirebaseAuthServices firebaseAuthServices;

  @override
  User? get currentUser => firebaseAuthServices.currentUser;

  @override
  Future<bool> hasSeenOnboarding() async {
    return await localStorageServices.getData(
          key: 'hasSeenOnboarding',
        ) ??
        false;
  }


  @override
  Future<bool> checkEmailVerification() async {
    return await firebaseAuthServices.checkEmailVerification();
  }
}
